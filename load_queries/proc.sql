...
begin
	...
	loop
		if (elem->>'likeTime') > latestLikeTime
		then
			latestLikeTime := (elem->>'likeTime');
			latestLikeId := (elem->>'id');
			latestLike := elem;
		elsif (elem->>'likeTime') = latestLikeTime
		then
			if (elem->>'id') < latestLikeId
			then
				latestLikeTime := (elem->>'likeTime');
				latestLikeId := (elem->>'id');
				latestLike := elem;
...
returns int8 as $$
	...
	where exists (select 1
				  ...
				  where post.id = x.id and p.id = person_id return 1) y);
$$ language sql;
